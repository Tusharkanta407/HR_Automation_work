"use client";

import { motion } from "motion/react";

export const GradientBackground = () => {
  return (
    <motion.div
      aria-hidden
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      transition={{ duration: 0.6 }}
      className="pointer-events-none fixed inset-x-0 bottom-0 z-0 h-[90vh] blur-gradient-bottom after:pointer-events-none after:absolute after:inset-0 after:bg-[url('data:image/svg+xml;base64,')] after:opacity-40 after:mix-blend-overlay"
      style={{
        background:
          "radial-gradient(1920px 100% at 50% 100%, rgba(255,30,84,0.6) 0%, rgba(80,56,255,0.5) 50%, transparent 100%)",
        maskImage:
          "radial-gradient(1920px 100% at 50% 100%, #000 0%, rgba(0, 0, 0, .5) 50%, transparent 100%)",
        WebkitMaskImage:
          "radial-gradient(1920px 100% at 50% 100%, #000 0%, rgba(0, 0, 0, .5) 50%, transparent 100%)",
      }}
    />
  );
};
